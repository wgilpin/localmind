...
Provide only the final answer. Do not show your thinking steps or any other text."""
        ...
        try:
            response = self.lmstudio_client.chat(
                ...
                top_p=0.9,
                max_tokens=2000  # Allow even more tokens for thinking + terms
            )
            ...
            for line in lines:
                ...
                # Check if it looks like a search term (1-6 words, no special chars)
                if line and 1 <= len(line.split()) <= 6 and not any(char in line for char in '<>[]{}()'):
