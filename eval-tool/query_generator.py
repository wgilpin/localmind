...
class QueryGenerator:
    ...
    def _extract_short_query(self, title: str, url: str) -> str:
        ...
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 
                     ...
                     'must', 'can', 'need', 'dare', 'ought', 'shall', 'uses', 'using', 'used',
                     'get', 'gets', 'getting', 'got', 'gotten', 'make', 'made', 'making',
                     'your', 'our', 'my', 'his', 'her', 'its', 'their', 'this', 'that', 'these',
                     'those', 'all', 'any', 'some', 'no', 'not', 'only', 'just', 'very', 'too'}
        ...
        if url:
            try:
                ...
                domain = re.sub(r'\.(com|org|net|io|edu|gov|co|uk|ai|app|dev).*$', '', domain)
                
                # If domain has meaningful parts, use them
                parts = re.split(r'[.-]', domain)
                # Filter out generic parts
                meaningful = [p for p in parts if len(p) > 2 and p not in {'www', 'blog', 'docs', 'api'}]
                if meaningful:
                    return meaningful[0]
            except:
                pass
    ...
    def generate_queries_for_bookmark(self, bookmark: Dict[str, Any], max_queries: int = 5) -> List[str]:
        ...
        longer_prompt = f"""Generate {num_queries - 1} different search queries (6 words maximum length for each) for this webpage.

Title: {title}
Content: {content[:500]}

List {num_queries - 1} queries only:""" if num_queries > 1 else None
        ...
        try:
            ...
            if longer_prompt and num_queries > 1:
                ...
                longer_queries = [q.strip() for q in longer_queries_text.split('\n') if q.strip()]
            ...
            for q in all_queries:
                ...
                if any(skip_phrase in q.lower() for skip_phrase in [
                    ...
                    'following are', 'below are', 'queries based on', 'search terms',
                    'keeping them to', 'word maximum', 'word limit', 'each within',
                    'limited to', 'words each'
                ]):
                    continue
